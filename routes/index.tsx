...
export const handler: Handlers<any,WithSession> = {
  async GET(_,cxt){
    ...
    if(entry){
      ...
      [reserve] = await select(
        "rsv_key,rsv_name,entry_start,entry_end from (select rsv_key,rsv_name,entry_start,entry_end from rel_reserve rr join rel_entry re on rr.entry_key = re.entry_key order by rsv_key desc limit 1)a where rr.state is null and (entry_start>?)and(entry_end is null or entry_end>?)",
